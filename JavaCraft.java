...
public class JavaCraft {
    ...
    private static final int CRAFTED_TORCH = 6;
    ...
    private static char UNICODE_WOOD = '\u2592';
    private static char UNICODE_LEAVES = '§';
    private static char UNICODE_STONE = '▓';
    private static char UNICODE_IRON_ORE = '°';
    private static char UNICODE_COAL_ORE = '°';
    private static char UNICODE_COIN = '$';
    private static char UNICODE_CRAFTED_TORCH = 'i';
    ...
    /**
     * function starts the game by:
     *  assigning values to attributes worldWidth and worldHeight
     *  defining size of the matrix world
     *  placing player in the middle of the board
     *  initializing inventory with empty ArrayList
     */
    ...
    /**
     * function assigns to empty blocks properties using pseudo-random numbers:
     *  Probabilities per each block:
     *      20% for placing wood
     *      15% for placing leaves
     *      15% for placing stone
     *      20% for placing iron ore
     *      30% for placing air
     */
    ...
    public static void displayWorld() {
        ...
        System.out.println(getBlockColor(world[0][0]) + "╔══" + "═".repeat(worldWidth * 2 - 2) + "╗");
        for (int y = 0; y < worldHeight; y++) {
            System.out.print(getBlockColor(world[0][y]) + "║");
            for (int x = 0; x < worldWidth; x++) {
    ...
    /**
     * @param blockType - an int representing blockType
     * @return a symbol with a color representing given blockType (i.e. °(with color white))
     */
    ...
    /**
     * @param blockType - an int representing blockType
     * @return a symbol representing given blockType (i.e. §, °)
     */
    private static char getBlockChar(int blockType) {
        switch (blockType) {
            case WOOD:
                return UNICODE_WOOD;
            case LEAVES:
                return UNICODE_LEAVES;
            case STONE:
                return UNICODE_STONE;
            case IRON_ORE:
                return UNICODE_IRON_ORE;
            case COAL_ORE:
                return UNICODE_COAL_ORE;
            case COIN:
                return UNICODE_COIN;
            case CRAFTED_TORCH:
                return UNICODE_CRAFTED_TORCH;
            default:
                return '-';
        }
    }
    ...
    /**
     * function collects an input from user and using conditional statements calls functions created to solve each case
     * this function is also responsible for secret door mechanic
     */
    //gameloop game loop
    ...
    /**
     * function removes all items in inventory and adds INVENTORY_SIZE items for each type of block
     */
    ...
    /**
     * function DOES NOT generate empty world
     * it generates new world with flag of Netherlands
     */
    ...
    /**
     * function adds (if block != air) a block to the player's inventory and removes it from the board
     */
    public static void mineBlock() {
        ...
        if (blockType != AIR) {
            ...
            if(getBlockName(blockType)=="Torch")
                System.out.println("Picked up " + getBlockName(blockType) + ".");
            else System.out.println("Mined " + getBlockName(blockType) + ".");
        } else {
    ...
    //DO NOT USE UNLES YOU WANT TO FIX
    ...
    public static void displayCraftingRecipes() {
        ...
        System.out.println("4. Craft Torch: 2 Wood, 1 Coal Ore");
    }
    ...
    {
        if(inventoryContains(COAL_ORE, 1) && inventoryContains(WOOD, 2))
        {
            removeItemsFromInventory(COAL_ORE, 1);
            removeItemsFromInventory(WOOD, 2);
            System.out.println("Crafted torch!");
            inventory.add(CRAFTED_TORCH);
    ...
    public static void interactWithWorld() {
        ...
        switch (blockType) {
            ...
            case COAL_ORE:
                ...
                break;
            case CRAFTED_TORCH:
                System.out.println("You picked up the torch from the floor.");
                inventory.add(CRAFTED_TORCH);
                break;
            case COIN:
    ...
    /**
     * @param blockType - an int representing a blockType
     * @return a string containing a name of a given block type
     */
    ...
    /**
     * function displays block names and corresponding symbols
     * using the same colors as game uses
     */
    public static void displayLegend() {
        System.out.println(ANSI_BLUE + "Legend:");
        System.out.println(ANSI_WHITE + "-- - Empty block");
        System.out.println(ANSI_RED + UNICODE_WOOD + " - Wood block");
        System.out.println(ANSI_GREEN + UNICODE_LEAVES + " - Leaves block");
        System.out.println(ANSI_BLUE + UNICODE_STONE + " - Stone block");
        System.out.println(ANSI_WHITE + UNICODE_IRON_ORE + " - Iron ore block");
        System.out.println(ANSI_PURPLE + UNICODE_COAL_ORE + " - Coal ore block");
        System.out.println(ANSI_YELLOW + UNICODE_COIN + " - Coin block");
        System.out.println(ANSI_YELLOW + UNICODE_CRAFTED_TORCH + " - Torch block");
        if (inSecretArea) System.out.println(ANSI_BLUE + "P - Player" + ANSI_RESET);
        else System.out.println(ANSI_GREEN + "P - Player" + ANSI_RESET);
    }

    /**
     * function iterates through inventory, craftedItems and outputs names with count for items in inventory,
     * but for craftedItems function only outputs names of crated items
     */
